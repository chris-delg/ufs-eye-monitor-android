...
// JNI function to run lsufs with command line arguments
JNIEXPORT jint JNICALL
Java_com_qualcomm_eyemonitorinterface_runLSUFS(JNIEnv *env, jclass clazz, jobjectArray args)
{
    int argc = 0;
    char **argv = NULL;
    jint result = -1;
    ...
    LOGI("Calling run_lsufs with %d arguments\n", argc);

    // Call the main function
    result = run_lsufs(argc, argv);

    LOGI("run_lsufs returned: %d\n", result);
...
JNIEXPORT jint JNICALL
Java_com_qualcomm_eyemonitorinterface_runLSUFSCommand(JNIEnv *env, jclass clazz, jstring command)
{
    const char *cmd_str;
    char *argv_buffer;
    char **argv;
    ...
    LOGI("Calling run_lsufs with command: %s\n", cmd_str);
